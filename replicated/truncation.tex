...
\begin{Boxfig}{Local conversion to $(2,2)$ additive shares}{fig:get-additive}
  ...
  Roles: Dealer, $P_1$, $P_2$ where $\vx.\mathsf{placement}$ is Host(Dealer) \newline
  With Host(Dealer):
  \begin{enumerate}
    \item $k \asn \mathsf{SampleKey}()$.
    \item $\seed \asn \derives(k, 0)$.
    \item $\vx^1 \asn \mathsf{Sample}(\vx.\mathsf{shape}, \seed)$.
    \item $\vx^2 \asn \vx - \vx^1$.
    \item Send $\vx^1$ to Plain($P_1$) and $\vx^2$ to Plain($P_2$).
  \end{enumerate}
  With Host($P_1$):
  \begin{enumerate}
      \item Receive $\vx^1$.
