...
$P_3$ does the following:
  \begin{enumerate}
    ...
    generate 2-out-of-2 additive shares by calling $\mathsf{GetAdditiveShare}()$
    on each entry and get $
    \vr\ui_{\mathsf{bot}}, \vr\ui_{\mathsf{msb}}, \vr\ui_{\mathsf{top}}$ for $i \in [0,1]$.

    Send the corresponding $\vr\ui$ to party $P_i$.
    \item Sample two seeds $\seed_i \asn \derives(k_3, k+i)$ for $i \in
    [1,2]$. and send $\seed_i$ to $P_i$. Use these to generate random numbers
    $y_1, y_2 \in \Z_{2^k}^{\mathsf{s}}$ by calling $y_i \asn \mathsf{Sample}(\seed_i,
    \mathsf{s})$
    \item Set own share of the truncation result as $(y_2, y_1)$.
 \end{enumerate}
...
\begin{enumerate}
   ...
   using the $2$-out-of-$2$ sharings of $\vx$ and the shares provided by $P_3$.
\end{enumerate}
...
\begin{Boxfig}{Generic truncation protocol.}{fig:truncpr-generic}
  ...
  \begin{enumerate}
    ...
    \item Compute $\share{\vb} \asn \share{\vr_{\mathsf{msb}}} \oplus (c / 2^{k-1})$. Note here that $\oplus$
...
\begin{Boxfig}{Opening protocol}{fig:open}
  {Protocol $\Proto{\Open}(\share{\vx})$}
  On $P_1$:
  \begin{enumerate}
    \item Send $\vx_1$ to $P_2$. Wait for $\vx_2$ from $P_2$.
    \item Output $\vx \asn \vx_1 + \vx_2$.
  \end{enumerate}
  On $P_2$:
  \begin{enumerate}
    \item Send $\vx_2$ to $P_1$. Wait for $\vx_1$ from $P_1$.
    \item Output $\vx \asn \vx_1 + \vx_2$.
  \end{enumerate}
\end{Boxfig}
